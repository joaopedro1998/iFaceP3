...
public class Main {
	...
	public static void main(String[] args) {
		...
		UsersPremium usersPremium = null;
		...
		while(working)
		{
			switch (flag) {
				...
				case Register:
					...
					{
						optionChoosed = menu.type();
						if(optionChoosed == 1)
							users.add(new Users(login,password,name));
						else
							usersPremium = new UsersPremium(login, password, name);
							users.add(usersPremium);
						flag = MenuStatus.Begin;
					}				
					break;
				case Inside:
					if(users.get(accountLogged).getClass() == UsersPremium.class)
						optionChoosed = menu.insidePremium();
					else
						optionChoosed = menu.insideFree();
					if(optionChoosed == 1)
				...
				case ManageCommunity:
					name = ((UsersPremium) users.get(accountLogged)).getOwnCommunity();
				...
				case CreateCommunity:
					...
					if(flag != MenuStatus.Inside)
					{
						...
						((UsersPremium) users.get(accountLogged)).setOwnCommunity(name);
						flag = MenuStatus.Inside;
					}
					break;
				case EditProfile:
				...
				case RecoverInformations:
					...
					if(optionChoosed == 1)
					{
						...
						if(users.get(accountLogged).getClass() == UsersPremium.class)
							((UsersPremium)users.get(accountLogged)).community();
